...
def _to_domain(record: PurchaseRequisitionRecord) -> Requisition:
    ...
    return Requisition(
        ...
        id=record.id, request_no=record.request_no, title=record.title,
        factory_code=record.factory_code, department=record.department,
        cost_center=record.cost_center, priority=record.priority,
        needed_date=record.needed_date, reason=record.reason, status=record.status,
        approval_comment=record.approval_comment, approved_by=record.approved_by,
        approved_at=record.approved_at, order_id=record.order_id,
        created_by=record.created_by, created_at=record.created_at,
        updated_at=record.updated_at,
        lines=[RequisitionLine(material_code=line.material_code, material_name=line.material_name, specification=line.specification, quantity=line.quantity, unit=line.unit, estimated_unit_price=line.estimated_unit_price) for line in record.lines],
    )
...
class RequisitionService:
    def save_spare_snapshot(self,db,record,codes):
        ...
        if not codes:
            if previous:db.delete(previous)
            return
        ...
        if set(codes)!={r['material_code'] for r in analysis['items']} or set(codes)!={l.material_code for l in record.lines}:
            raise HTTPException(422,'备件计划物料必须仍为有效备件，并与申请明细完全一致')
        analysis['captured_at']=datetime.now(UTC).isoformat()
        analysis['requested_quantities']={l.material_code:float(l.quantity) for l in record.lines}
        if not previous:
            previous=SpareRequestSnapshot(request_id=record.id);db.add(previous)
        previous.payload=json.dumps(analysis,ensure_ascii=False)
        db.flush()
    def list(self, db: Session, page: int, page_size: int, keyword: str = "", status: str = "") -> tuple[list[Requisition], int]:
        filters = []
        if keyword:
            pattern = f"%{keyword.strip()}%"
            filters.append(or_(PurchaseRequisitionRecord.request_no.ilike(pattern), PurchaseRequisitionRecord.title.ilike(pattern), PurchaseRequisitionRecord.department.ilike(pattern)))
        if status:
            filters.append(PurchaseRequisitionRecord.status == status)
        total = db.scalar(select(func.count()).select_from(PurchaseRequisitionRecord).where(*filters)) or 0
        statement = select(PurchaseRequisitionRecord).where(*filters).options(selectinload(PurchaseRequisitionRecord.lines)).order_by(PurchaseRequisitionRecord.created_at.desc()).offset((page - 1) * page_size).limit(page_size)
    ...
    def get_record(self, db: Session, item_id: str) -> PurchaseRequisitionRecord | None:
        return db.scalar(select(PurchaseRequisitionRecord).where(PurchaseRequisitionRecord.id == item_id).options(selectinload(PurchaseRequisitionRecord.lines)))
    ...
    def create(self, db: Session, payload: RequisitionCreate, created_by: str) -> Requisition:
        ...
        record = PurchaseRequisitionRecord(request_no=f"PR-{sequence + 1:06d}", title=payload.title, factory_code=payload.factory_code, department=payload.department, cost_center=payload.cost_center, priority=payload.priority, needed_date=payload.needed_date, reason=payload.reason, created_by=created_by)
        _replace_lines(record, payload.lines)
        db.add(record); db.flush(); db.refresh(record, attribute_names=["lines"])
        self.save_spare_snapshot(db,record,payload.spare_plan_codes)
        return _to_domain(record)

    def update(self, db: Session, item_id: str, payload: RequisitionUpdate) -> tuple[Requisition | None, str | None]:
        ...
        if lines is not None:
            ...
            if codes is None:
                previous=db.get(SpareRequestSnapshot,record.id)
                if previous:codes=[r['material_code'] for r in json.loads(previous.payload)['items']]
        if codes is not None:
            self.save_spare_snapshot(db,record,codes)
        record.status = RequisitionStatus.DRAFT
        record.approval_comment = ""
        db.flush(); db.refresh(record, attribute_names=["lines"])
    ...
    def decide(self, db: Session, item_id: str, approved: bool, comment: str, approver: str) -> tuple[Requisition | None, str | None]:
        record = self.get_record(db, item_id)
        if record is None:
            return None, "not_found"
    ...
    def convert(self, db: Session, item_id: str, payload: RequisitionConversion, created_by: str):
        ...
        order_payload = PurchaseOrderCreate(
            ...
            factory_code=record.factory_code, currency=payload.currency,
            template_id=payload.template_id, payment_terms=payload.payment_terms,
            delivery_address=payload.delivery_address,
            lines=[OrderLineInput(material_code=line.material_code, material_name=line.material_name, specification=line.specification, quantity=line.quantity, unit=line.unit, unit_price=line.estimated_unit_price, delivery_date=record.needed_date) for line in record.lines],
        )
